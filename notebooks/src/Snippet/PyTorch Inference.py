...
# MAGIC 
# MAGIC 一方で、今回のような"モデル"オブジェクトをメモリ上にロードして、クラスタ上で利用するには、データを別のクラスタ(のメモリ上)にもロードさせる必要があります。そのための仕組みが「broadcast変数」であり、下記のコードの通り、
# MAGIC 
# MAGIC * `sc.broadcast()`で変数をbroadcastでき、
# MAGIC * 各ノード上では、上記のbroadcast変数名でアクセス可能できる
# MAGIC 
# MAGIC ようになります。
